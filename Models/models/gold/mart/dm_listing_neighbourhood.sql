...
changes as (
    select
        ...
        round(
            (active_listings - lag(active_listings) over (partition by listing_neighbourhood order by month_year))
            / nullif(lag(active_listings) over (partition by listing_neighbourhood order by month_year), 0)::numeric * 100,
            2
        ) as percentage_change_active_listings,
        round(
            (inactive_listings - lag(inactive_listings) over (partition by listing_neighbourhood order by month_year))
            / nullif(lag(inactive_listings) over (partition by listing_neighbourhood order by month_year), 0)::numeric * 100,
            2
